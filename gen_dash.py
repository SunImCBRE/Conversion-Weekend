...
function openSVMo(i){
  ...
  document.getElementById('svMoB').innerHTML=`
    ...
    ${(r.issueNotes&&r.issueNotes.length)?`<div class="ms-sec"><h4>Issues Noted (${r.issueNotes.length})</h4><div style="display:flex;flex-direction:column;gap:5px">${r.issueNotes.map(n=>{const[area,...rest]=n.split(': ');return`<div style="background:var(--card2);border-radius:5px;padding:7px 10px;font-size:.76rem"><span style="color:var(--53gl);font-weight:700">${area}:</span> <span style="color:var(--white)">${rest.join(': ')}</span></div>`}).join('')}</div></div>`:'<div class="ms-sec"><h4>Issues Noted</h4><div style="color:var(--muted);font-size:.76rem">No issues recorded</div></div>'}
